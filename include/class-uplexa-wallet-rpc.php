 ...
 * Modified to work with uplexa-wallet-rpc wallet by mosu-forge
 */

defined( 'ABSPATH' ) || exit;

class uPlexa_Wallet_Rpc
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
            if(is_admin())
                echo '[ERROR] Failed to connect to uplexa-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    public function make_uri($address, $amount, $recipient_name = null, $description = null)
    {
        ...
        $new_amount = $amount * UPLEXA_GATEWAY_ATOMIC_UNITS_POW;

<?php
/*
 * Main Gateway of uPlexa using either a local daemon or the explorer
 * Authors: Serhack, cryptochangements, mosu-forge
 */

defined( 'ABSPATH' ) || exit;

require_once('class-uplexa-cryptonote.php');

class uPlexa_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'uplexa_gateway';
    private static $_title = 'uPlexa Gateway';
    private static $_method_title = 'uPlexa Gateway';
    private static $_method_description = 'uPlexa Gateway Plug-in for WooCommerce.';
    ...
    private static $use_uplexa_price = false;
    private static $use_uplexa_price_decimals = UPLEXA_GATEWAY_ATOMIC_UNITS;

    private static $cryptonote;
    private static $uplexa_wallet_rpc;
    private static $uplexa_explorer_tools;
    private static $log;
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.UPLEXA_GATEWAY_PLUGIN_URL.'assets/images/uplexa-icon.png"/>', $this->id); //$this->id
    }
    ...
    function __construct($add_action=true)
    {
        $this->id = self::$_id;
	//$this->id = self::get_id();
        $this->method_title = __(self::$_method_title, 'uplexa_gateway');
        $this->method_description = __(self::$_method_description, 'uplexa_gateway');
        ...
        self::$address = $this->settings['uplexa_address'];
        ...
        self::$use_uplexa_price = $this->settings['use_uplexa_price'] == 'yes';
        self::$use_uplexa_price_decimals = $this->settings['use_uplexa_price_decimals'];

        $explorer_url = self::$testnet ? UPLEXA_GATEWAY_TESTNET_EXPLORER_URL : UPLEXA_GATEWAY_MAINNET_EXPLORER_URL;
        defined('UPLEXA_GATEWAY_EXPLORER_URL') || define('UPLEXA_GATEWAY_EXPLORER_URL', $explorer_url);

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));
        // Initialize helper classes
        self::$cryptonote = new uPlexa_Cryptonote();
        if(self::$confirm_type == 'uplexa-wallet-rpc') {
            require_once('class-uplexa-wallet-rpc.php');
            self::$uplexa_wallet_rpc = new uPlexa_Wallet_Rpc(self::$host, self::$port);
        } else {
            require_once('class-uplexa-explorer-tools.php');
            self::$uplexa_explorer_tools = new uPlexa_Explorer_Tools(self::$testnet);
        }
    ...
    {
        $this->form_fields = include 'admin/uplexa-gateway-admin-settings.php';
    }

    public function validate_uplexa_address_field($key,$address)
    {
        if($this->settings['confirm_type'] == 'viewkey') {
            if (strlen($address) == 98 && substr($address, 0, 1) == 'U')
                if(self::$cryptonote->verify_checksum($address))
                    return $address;
            self::$_errors[] = 'uPlexa address is invalid';
        }
        return $address;
    }
    ...
    public function admin_options()
    {
        ...
        if($confirm_type === 'uplexa-wallet-rpc')
        ...
        include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$uplexa_wallet_rpc->getbalance();
        $height = self::$uplexa_wallet_rpc->getheight();
        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to uplexa-wallet-rpc';
            self::$log->add('uPlexa_Payments', '[ERROR] Cannot connect to uplexa-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_uplexa($wallet_amount['balance']).' uPlexa',
                'unlocked_balance' => self::format_uplexa($wallet_amount['unlocked_balance']).' uPlexa'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if(self::$confirm_type != 'uplexa-wallet-rpc') {
          ...
          // Generate subaddress
          $payment_id = self::$uplexa_wallet_rpc->create_address(0, 'Order: ' . $order_id);
          if(isset($payment_id['address'])) {
            $payment_id = $payment_id['address'];
          }
          else {
            $this->log->add('uPlexa_Gateway', 'Couldn\'t create subaddress for order ' . $order_id);
          }
        }
        ...
        $uplexa_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $uplexa_amount = $uplexa_amount - $uplexa_amount * self::$discount / 100;

        $uplexa_amount = intval($uplexa_amount * UPLEXA_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $uplexa_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'uplexa_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        //$api_link = 'https://min-api.cryptocompare.com/data/price?fsym=UPX&tsyms='.$currencies.'&extraParams=uplexa_woocommerce';
	$api_link = "https://uplexa.com/data?currencies={$currencies}";
        $curl = curl_init();
        ...
        if(self::$confirm_type == 'uplexa-wallet-rpc')
            ...
            $height = self::$uplexa_explorer_tools->getheight();
        set_transient('uplexa_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'uplexa_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'uplexa_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_uplexa = $quote->amount_total;

$fp = fopen('/var/www/html/wordpress/wp-content/debuggingtwo.txt', 'w');
fwrite($fp, $payment_id);
fclose($fp);
...
$fp = fopen('/var/www/html/wordpress/wp-content/debugging.txt', 'w');
fwrite($fp, print_r($new_txs, true));
fclose($fp);
            ...
            $paid = $amount_paid > $amount_uplexa - UPLEXA_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('uPlexa_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'uplexa_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'uplexa_gateway'));
                }

            } else if($paid) {
                self::$log->add('uPlexa_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('uPlexa_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'uplexa_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $address_index = self::$uplexa_wallet_rpc->get_address_index($subaddress);
        if(isset($address_index['index']['minor'])){
          $address_index = $address_index['index']['minor'];
        }
        else {
          self::$log->add('uPlexa_Gateway', '[ERROR] Couldn\'t get address index of subaddress: ' . $subaddress);
          return $txs;
        }
        $payments = self::$uplexa_wallet_rpc->get_transfers(array( 'in' => true, 'pool' => true, 'subaddr_indices' => array($address_index)));
    ...
    {
        $txs = array();
        $outputs = self::$uplexa_explorer_tools->get_outputs(self::$address, self::$viewkey);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_uplexa($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('uplexa_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * UPLEXA_GATEWAY_DIFFICULTY_TARGET);
            ...
            if(self::$confirm_type == 'uplexa-wallet-rpc') {
                ...
                if ($address) {
                    ...
                    self::$log->add('uPlexa_Gateway', '[ERROR] Merchant has not set uPlexa address');
                    return '[ERROR] Merchant has not set uPlexa address';
                }
            }
            ...
            $amount_formatted = self::format_uplexa($amount_due);
            $qrcode_uri = 'uplexa:'.$address.'?tx_amount='.$amount_formatted.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_uplexa($amount_total),
                'amount_paid_formatted' => self::format_uplexa($amount_paid),
                'amount_due_formatted' => self::format_uplexa($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with uPlexa'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/admin/order-history-error-page.php';
            return;
        }
        include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('uplexa_gateway_payment_details');
        include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/customer/order-email-error-block.php';
            return;
        }
        include UPLEXA_GATEWAY_PLUGIN_DIR . '/templates/uplexa-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_uplexa_price()
    {
        return self::$use_uplexa_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $uplexa_amount = intval(UPLEXA_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / UPLEXA_GATEWAY_ATOMIC_UNITS_POW;
        $uplexa_amount_formatted = sprintf('%.'.self::$use_uplexa_price_decimals.'f', $uplexa_amount);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $uplexa_amount_formatted
            <span class="woocommerce-Price-currencySymbol">UPX</span>
        </span>
...
HTML;
    ...
    public static function convert_wc_price_order($price_html, $order)
    {
        ...
        $uplexa_amount = intval(UPLEXA_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / UPLEXA_GATEWAY_ATOMIC_UNITS_POW;
        $uplexa_amount_formatted = sprintf('%.'.UPLEXA_GATEWAY_ATOMIC_UNITS.'f', $uplexa_amount);
...
HTML;
    ...
    public static function format_uplexa($atomic_units) {
        return sprintf(UPLEXA_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / UPLEXA_GATEWAY_ATOMIC_UNITS_POW);
    }
